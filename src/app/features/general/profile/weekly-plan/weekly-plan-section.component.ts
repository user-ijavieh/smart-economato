...
export class WeeklyPlanSectionComponent implements OnInit, OnChanges {
  ...
  confirmSlot(slot: WeeklyPlanSlotResponse): void {
    ...
    this.weeklyPlanService.confirmSlot(plan.id, slot.id).subscribe({
      next: () => {
        this.messageService.showSuccess('Slot confirmado.');
  ...
  cancelSlot(slot: WeeklyPlanSlotResponse): void {
    ...
    this.weeklyPlanService.cancelSlot(plan.id, slot.id).subscribe({
      next: () => {
        this.messageService.showSuccess('Slot cancelado.');
        this.reloadPlan(plan.id);
      },
  ...
  confirmDay(dayOfWeek: number): void {
    ...
    this.weeklyPlanService.confirmDay(plan.id, dayOfWeek).subscribe({
      next: () => {
        this.messageService.showSuccess('Día confirmado.');
        this.reloadPlan(plan.id);
      },
  ...
  cancelStudent(event: { slot: WeeklyPlanSlotResponse; student: WeeklyPlanSlotStudentResponse }): void {
    ...
    this.weeklyPlanService.cancelStudentFromSlot(plan.id, event.slot.id, event.student.studentId).subscribe({
      next: () => {
        this.messageService.showSuccess('Alumno cancelado.');
        this.reloadPlan(plan.id);
      },
      error: (error) => this.messageService.showError(error.error?.message || 'No se pudo cancelar el alumno.')
    });
  }
