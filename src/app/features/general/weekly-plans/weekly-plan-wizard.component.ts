  ...
  save() {
    ...
    if (this.editMode && this.planId) {
      this.weeklyPlanService.updatePlan(this.planId, request).subscribe({
        ...
        error: (err) => {
          this.saving = false;
          this.messageService.showError(err.error?.message || 'Error al actualizar el plan semanal.');
        }
      });
    } else {
      this.weeklyPlanService.createPlan(request).subscribe({
        ...
        error: (err) => {
          this.saving = false;
          this.messageService.showError(err.error?.message || 'Error al guardar el plan semanal.');
        }
      });
    }
  }
