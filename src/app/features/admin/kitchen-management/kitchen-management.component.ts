...
export class KitchenManagementComponent implements OnInit, OnDestroy {
  ...
  applyServerFilters(): void {
    const hasRecipeFilter = this.recipeNameFilter.trim().length > 0;
    const hasUserFilter = this.userNameFilter.trim().length > 0;
    ...
    if (hasSearchTerm) {
      // Búsqueda general por nombre de receta tiene prioridad
      filterObservable = this.kitchenService.searchCookingAuditsByName(this.searchTerm);
    } else if (hasRecipeFilter) {
      filterObservable = this.kitchenService.searchCookingAuditsByName(this.recipeNameFilter);
    } else if (hasUserFilter) {
      filterObservable = this.kitchenService.searchCookingAuditsByUserName(this.userNameFilter);
    } else {
        ...
        next: (audits) => {
          ...
          if (activeFilters.length > 1) {
            if (hasRecipeFilter && !activeFilters.includes('recipe') === false) {
              const filterText = this.recipeNameFilter.toLowerCase();
              filtered = filtered.filter(a => a.recipeName?.toLowerCase().includes(filterText));
            }
            if (hasUserFilter && !activeFilters.includes('user') === false) {
              const filterText = this.userNameFilter.toLowerCase();
              filtered = filtered.filter(a => a.userName?.toLowerCase().includes(filterText));
            }
  ...
  private performSearch(): void {
    ...
    const hasOtherFilters = this.recipeNameFilter || this.userNameFilter || 
  ...
  hasActiveHistoryFilters(): boolean {
    return this.searchTerm.trim().length > 0
      || this.recipeNameFilter.trim().length > 0
      || this.userNameFilter.trim().length > 0
