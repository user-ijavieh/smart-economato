...
export class UsersManagementComponent implements OnInit, OnDestroy {
    ...
    async toggleUserVisibility(user: User): Promise<void> {
        const actionText = this.showingHidden ? 'activando' : 'desactivando';
        ...
        this.userService.toggleHidden(user.id, !this.showingHidden).subscribe({
            ...
            error: (err) => {
                // 1. Log interno con el servicio Logger (lo que traía master)
                this.logger.error(`Error ${actionText} usuario:`, err);
                
                // 2. Determinar la clave de traducción según el estado (lo que tenías tú)
                const errorKey = this.showingHidden ? 'USERS.MESSAGES.ACTIVATE_ERROR' : 'USERS.MESSAGES.DEACTIVATE_ERROR';
                
                // 3. Priorizar el mensaje del servidor si existe, si no, usar la traducción
                const errorMessage = err.error?.message || this.translate.instant(errorKey);
                
                // 4. Mostrar el mensaje al usuario
